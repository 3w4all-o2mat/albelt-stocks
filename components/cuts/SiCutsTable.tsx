...
export default function SiCutsTable({
  cuts,
  deleteHours,
  userId,
  userRole,
            ...
            {cuts.length === 0 && (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-slate-500">
                  Aucun stock initial pour le moment.
                </td>
              </tr>
            )}
            {cuts.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50">
                <td className="px-5 py-3 font-medium text-slate-900">
                  <button
                    onClick={() => openModal(c)}
                    className="text-si hover:text-si/80 hover:underline transition-colors text-left"
