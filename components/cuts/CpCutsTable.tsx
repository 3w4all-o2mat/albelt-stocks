...
export default function CpCutsTable({
  cuts,
  deleteHours,
  userId,
  userRole,
    ...
    <>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-red-500 text-xs uppercase text-white">
            <tr>
            ...
            {cuts.length === 0 && (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-slate-500">
                  Aucune chute perdue pour le moment.
                </td>
              </tr>
            )}
            {cuts.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50">
                <td className="px-5 py-3 font-medium text-slate-900">
                  <button
                    onClick={() => openModal(c)}
                    className="text-red-500 hover:text-red-600 hover:underline transition-colors text-left"
